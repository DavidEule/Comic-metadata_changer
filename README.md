...
A Python GUI tool for bulk-editing comic metadata inside `.cbz` and `.cbr` archives. The program writes and updates `ComicInfo.xml` files using the ComicRack metadata standard.
...
This editor writes metadata inside each archive as a `ComicInfo.xml` file. Different readers handle embedded metadata differently.
...
For more Info go to the linked repository, theres a video there explaining what to do in koreader
...
Most desktop comic readers natively read `ComicInfo.xml`. Your updated metadata should appear immediately.
